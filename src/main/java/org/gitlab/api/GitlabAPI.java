...
public class GitlabAPI {
     ...
     * @param action An event used to close or re-open the issue.
     * @return The updated issue.
     * @throws IOException
     */
    public GitlabIssue editIssue(GitlabIssue issue, GitlabIssue.Action action)
            throws IOException {
        String labels = "";
        if (issue.getLabels() != null) {
            labels = String.join(",", issue.getLabels());
        }
        int milestoneId = 0;
        if (issue.getMilestone() != null) {
            milestoneId = issue.getMilestone().getId();
        }
        return editIssue(issue.getProjectId(),
                issue.getId(),
